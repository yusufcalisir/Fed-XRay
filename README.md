...
- **FFA-LoRA (Federated Freeze-A LoRA):** Freezes matrix $A$ globally across all institutions ($\nabla_A \mathcal{L} = 0$), strictly preserving linearity ($\bar{B}A \equiv \sum p_k B_k A$) and enabling multiplication-free Homomorphic Encryption ($\text{Multiplication Depth}=0$).
...
- **FlexLoRA SVD Aggregator:** Reconstructs full weight updates $\bar{\Delta W} = \sum p_k B_k A_k$ and computes Truncated Singular Value Decomposition (SVD):
  $$\bar{\Delta W} = U_r \Sigma_r V_r^\top \implies \bar{B} = U_r \Sigma_r^{1/2}, \quad \bar{A} = \Sigma_r^{1/2} V_r^\top$$
...
- **FedDyn (Dynamic Objective Regularization):** Dynamic parameter alignment tracking local gradient displacement with central server state $h^{t+1} = h^t - \alpha \frac{1}{K}\sum (\theta_k - \bar{\theta}^t)$.
- **FedOpt / FedAdam:** Adaptive server momentum on aggregated pseudo-gradients $\Delta^t = \sum p_k (\theta_k - \bar{\theta}^t)$.
...
- **FedProx:** Proximal parameter regularization $\frac{\mu}{2}(\|A_k - \bar{A}^t\|_F^2 + \|B_k - \bar{B}^t\|_F^2)$.
...
- **Dispersion-Weighted Prototype Synthesis (FedProto):** Eliminates orthogonal rotation collapse $Q \in O(D)$ by grounding class centroids in frozen foundation representations, weighted by intra-class covariance traces:
  $$\Sigma_{k,c} = \frac{1}{n_{k,c}}\sum_{i: y_i = c}(f(x_i) - p_{k,c})(f(x_i) - p_{k,c})^\top, \quad \alpha_{k,c} = \frac{\frac{n_{k,c}}{\operatorname{Tr}(\Sigma_{k,c}) + \epsilon}}{\sum_{j} \frac{n_{j,c}}{\operatorname{Tr}(\Sigma_{j,c}) + \epsilon}}, \quad \bar{p}_c = \sum_{k} \alpha_{k,c} p_{k,c}$$
- **Multimodal Semantic Prototypes:** Invariant text coordinate anchoring: $p_c = \lambda \left(\sum \alpha_{k,c} p_{c,k}^{\text{img}}\right) + (1-\lambda) p_c^{\text{txt}}$.
...
- **Layer 2 (Output Model Privacy):** Strict Patient-Level Gaussian Differential Privacy with Rényi Differential Privacy (RDP) composition accounting, ensuring strict $(\epsilon \le 2.0, \delta \le 10^{-5})$-DP bounds:
  $$g_i = \frac{1}{m_i}\sum_{j=1}^{m_i} g_{i,j}, \quad \bar{g}_i = g_i \cdot \min\left(1, \frac{C_{\text{patient}}}{\|g_i\|_2}\right), \quad \tilde{g} = \frac{1}{|\mathcal{B}_P|}\left(\sum_{i \in \mathcal{B}_P}\bar{g}_i + \mathcal{N}(0, \sigma^2 C_{\text{patient}}^2 I)\right)$$
...
- **Medical PDF Report Engine & Voice Briefing:** Single-page A4 diagnostic reports and real-time synthesized voice dictation in English and Turkish.
...
│   ├── src/app/                   # App Router, Layout & Telemetry
│   ├── src/components/            # Responsive Clinical Cockpit Panels
│   └── src/lib/                   # API Client, Types & Translations (EN / TR)
├── src/
...
```bash
# Execute all 43 unit and integration tests across the entire repository
