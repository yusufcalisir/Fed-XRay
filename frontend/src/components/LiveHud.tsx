...
export default function LiveHud({ numHospitals, isDefenseActive, isTrained }: LiveHudProps) {
  ...
  return (
    <div className="card-inner flex flex-wrap items-center justify-between gap-x-6 gap-y-2 px-5 py-3 mb-6 text-[11px] font-medium text-[var(--text-muted)]">
      ...
      <div className="flex items-center gap-2">
        <ShieldCheck className="w-3.5 h-3.5 text-accent-500 dark:text-accent-400" />
        <span>{isDefenseActive ? t("live_hud_shield") : t("sec2_shield_off")}</span>
      </div>
      <div className="flex items-center gap-2">
        <CheckCircle2 className={`w-3.5 h-3.5 ${isTrained ? "text-accent-500" : "text-slate-400"}`} />
